...
TIM_HandleTypeDef g_tim5_handle;      /* 定时器1句柄 */
...
void remote_init(void)
{
    ...
    g_tim5_handle.Instance = REMOTE_IN_TIMX;                    /* 通用定时器5 */
    g_tim5_handle.Init.Prescaler = (72 - 1);                    /* 预分频器,1M的计数频率,1us加1 */
    g_tim5_handle.Init.CounterMode = TIM_COUNTERMODE_UP;        /* 向上计数器 */
    g_tim5_handle.Init.Period = 10000;                          /* 自动装载值 */
    ...
    tim_ic_init_handle.ICSelection = TIM_ICSELECTION_DIRECTTI;  /* 映射到TI2上 */
    tim_ic_init_handle.ICPrescaler = TIM_ICPSC_DIV1;            /* 配置输入分频，不分频 */
    tim_ic_init_handle.ICFilter = 0x03;                         /* IC1F=0003 8个定时器时钟周期滤波 */
    HAL_TIM_IC_ConfigChannel(&g_tim5_handle, &tim_ic_init_handle, REMOTE_IN_TIMX_CHY);/* 配置TIM5通道2 */
    HAL_TIM_IC_Start_IT(&g_tim5_handle, REMOTE_IN_TIMX_CHY);    /* 开始捕获TIM5的通道2 */
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == REMOTE_IN_TIMX)
    {
        ...
        if (RDATA)      /* 上升沿捕获 */ 
        {   
            __HAL_TIM_SET_CAPTUREPOLARITY(&g_tim5_handle, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_FALLING);  /* CC2P=1 设置为下降沿捕获 */
            __HAL_TIM_SET_COUNTER(&g_tim5_handle, 0);  /* 清空定时器计数器值 */
            g_remote_sta |= 0X10;                      /* 标记上升沿已经被捕获 */
        }
        else           /* 下降沿捕获 */
        {
            dval = HAL_TIM_ReadCapturedValue(&g_tim5_handle, REMOTE_IN_TIMX_CHY);   /* 读取CCR1也可以清CC1IF标志位 */ 
            __HAL_TIM_SET_CAPTUREPOLARITY(&g_tim5_handle, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_RISING); /* 设置为上升沿捕获 */
